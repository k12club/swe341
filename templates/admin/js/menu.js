...
function change_icon(val){
	...
	jQuery.ajax({
		...
		global: false,
		type: "POST",
		data: ({template : val, check_key : "apphpsc"}),
		dataType: "html",
		async:false,
		error: function(html){
...
function open_popup(file, key_1, key_2){
	jQuery.ajax({
		...
		global: false,
		type: "POST",
		data: ({param : key_1, id : key_2, check_key : "apphpsc"}),
		dataType: "html",
		async:false,
		error: function(html){
			alert("AJAX: cannot connect to the server or server response error! Please try again later.");
		...
		success: function(html){
			var obj = jQuery.parseJSON(html);            			
			if(obj.status == "1"){
				var new_window = window.open('','PopupWindow','height=500,width=600,scrollbars=yes,screenX=600,screenY=100,toolbar=no,location=no,menubar=no',false);				
