...
if($objLogin->IsLoggedInAsAdmin()){
    ...
    if($objLogin->IsLoggedInAs('owner', 'mainadmin') && (file_exists('install.php') || file_exists('install/'))){
    	$actions_msg[] = '<span class="alert">'._INSTALL_PHP_EXISTS.'</span>';
    }

	if(SITE_MODE == 'development') $actions_msg[] = '<span class="alert">'._SITE_DEVELOPMENT_MODE_ALERT.'</span>';
	
	// test mode alert
	if(Modules::IsModuleInstalled('shopping_cart')){
        if(ModulesSettings::Get('shopping_cart', 'mode') == 'TEST MODE'){
			$actions_msg[] = '<span class="alert">'._TEST_MODE_ALERT.'</span>';
        }        
    }
    
    if($objLogin->IsLoggedInAs('owner', 'mainadmin')){
		$arr_folders = array('images/uploads/', 'images/flags/', 'images/banners/', 'images/gallery/', 'images/categories/', 'images/manufacturers/', 'images/products/', 'tmp/backup/', 'tmp/cache/', 'tmp/logs/', 'feeds/');
