...
if($objLogin->IsLoggedInAsAdmin() && 
  (Modules::IsModuleInstalled('shopping_cart') && (ModulesSettings::Get('shopping_cart', 'is_active') == 'yes'))
){
	...
	if($tabid == '1_1') {		
		$first_tab_content = '<script type="text/javascript">
				...
				$selStatType = 'COUNT(*)';
				$join_clause = ' LEFT OUTER JOIN '.TABLE_CUSTOMERS.' cust ON o.customer_id = cust.id ';  
				$where_clause = ' AND o.status > 1 '.(($country_id != '0') ? ' AND cust.b_country = \''.$country_id.'\'' : '');
		...
		$first_tab_content .= get_chart_changer('1_1', $chart_type, $year, $country_id, 'mod_shop_orders_statistics');		
		...
		$second_tab_content .= get_chart_changer('1_2', $chart_type, $year, $country_id, 'mod_shop_orders_statistics');		
		...
		$third_tab_content = '<script type="text/javascript">
				...
				// calculate summ according to default currency
				$selStatType = 'FORMAT(SUM(o.vat_fee * c.rate), 2) ';
				$join_clause = ' INNER JOIN '.TABLE_CURRENCIES.' c ON o.currency = c.code ';
				$join_clause .= ' INNER JOIN '.TABLE_CUSTOMERS.' cust ON o.customer_id = cust.id ';  
				$where_clause = ' AND o.status = 2 '.(($country_id != '0') ? ' AND cust.b_country = \''.$country_id.'\'' : '');
	
				$sql = 'SELECT
				  (SELECT '.$selStatType.' FROM '.TABLE_ORDERS.' o '.$join_clause.' WHERE SUBSTRING(o.payment_date, 6, 2) = \'01\' AND SUBSTRING(o.payment_date, 1, 4) = '.(int)$year.' '.$where_clause.') as month1,
				  (SELECT '.$selStatType.' FROM '.TABLE_ORDERS.' o '.$join_clause.' WHERE SUBSTRING(o.payment_date, 6, 2) = \'02\' AND SUBSTRING(o.payment_date, 1, 4) = '.(int)$year.' '.$where_clause.') as month2,
				  (SELECT '.$selStatType.' FROM '.TABLE_ORDERS.' o '.$join_clause.' WHERE SUBSTRING(o.payment_date, 6, 2) = \'03\' AND SUBSTRING(o.payment_date, 1, 4) = '.(int)$year.' '.$where_clause.') as month3,
				  (SELECT '.$selStatType.' FROM '.TABLE_ORDERS.' o '.$join_clause.' WHERE SUBSTRING(o.payment_date, 6, 2) = \'04\' AND SUBSTRING(o.payment_date, 1, 4) = '.(int)$year.' '.$where_clause.') as month4,
				  (SELECT '.$selStatType.' FROM '.TABLE_ORDERS.' o '.$join_clause.' WHERE SUBSTRING(o.payment_date, 6, 2) = \'05\' AND SUBSTRING(o.payment_date, 1, 4) = '.(int)$year.' '.$where_clause.') as month5,
				  (SELECT '.$selStatType.' FROM '.TABLE_ORDERS.' o '.$join_clause.' WHERE SUBSTRING(o.payment_date, 6, 2) = \'06\' AND SUBSTRING(o.payment_date, 1, 4) = '.(int)$year.' '.$where_clause.') as month6,
				  (SELECT '.$selStatType.' FROM '.TABLE_ORDERS.' o '.$join_clause.' WHERE SUBSTRING(o.payment_date, 6, 2) = \'07\' AND SUBSTRING(o.payment_date, 1, 4) = '.(int)$year.' '.$where_clause.') as month7,
				  (SELECT '.$selStatType.' FROM '.TABLE_ORDERS.' o '.$join_clause.' WHERE SUBSTRING(o.payment_date, 6, 2) = \'08\' AND SUBSTRING(o.payment_date, 1, 4) = '.(int)$year.' '.$where_clause.') as month8,
				  (SELECT '.$selStatType.' FROM '.TABLE_ORDERS.' o '.$join_clause.' WHERE SUBSTRING(o.payment_date, 6, 2) = \'09\' AND SUBSTRING(o.payment_date, 1, 4) = '.(int)$year.' '.$where_clause.') as month9,
				  (SELECT '.$selStatType.' FROM '.TABLE_ORDERS.' o '.$join_clause.' WHERE SUBSTRING(o.payment_date, 6, 2) = \'10\' AND SUBSTRING(o.payment_date, 1, 4) = '.(int)$year.' '.$where_clause.') as month10,
				  (SELECT '.$selStatType.' FROM '.TABLE_ORDERS.' o '.$join_clause.' WHERE SUBSTRING(o.payment_date, 6, 2) = \'11\' AND SUBSTRING(o.payment_date, 1, 4) = '.(int)$year.' '.$where_clause.') as month11,
				  (SELECT '.$selStatType.' FROM '.TABLE_ORDERS.' o '.$join_clause.' WHERE SUBSTRING(o.payment_date, 6, 2) = \'12\' AND SUBSTRING(o.payment_date, 1, 4) = '.(int)$year.' '.$where_clause.') as month12
				  FROM '.TABLE_ORDERS.'
				  GROUP BY month1';         
		...
		$third_tab_content .= get_chart_changer('1_3', $chart_type, $year, $country_id, 'mod_shop_orders_statistics');		
	...
	$tabs = new Tabs(1, 'xp', TABS_DIR, '?admin=mod_shop_orders_statistics');   
	...
	$tab3=$tabs->AddTab(_ORDERS.' ('._TAXES.')', $third_tab_content);
	$tab4=$tabs->AddTab(_ORDERS.' ('._MAP_OVERLAY.')', $fourth_tab_content);
	...
	draw_title_bar(prepare_breadcrumbs(array(_SHOPPING_CART=>'',_STATISTICS=>'')));
