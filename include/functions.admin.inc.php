 ...
 *  	@param $page
 **/
function get_chart_changer($tabid, $chart_type, $year, $country_id = '-1', $page = 'accounts_statistics')
{
...
function draw_set_values($result, $chart_type, $chart_name, $pre_addition = '')
{
	...
	$output .= $nl.' data.setValue(6, 1, '.str_replace(',', '', $res_month7).');';
	...
	if($chart_type == 'barchart'){
		$output .= $nl.'new google.visualization.BarChart(document.getElementById("div_visualization")).draw(data, {is3D: true, min:0, title:"'.$chart_name.'"});'; 
	}else if($chart_type == 'piechart'){
		$output .= $nl.'new google.visualization.PieChart(document.getElementById("div_visualization")).draw(data, {is3D: true, min:0, title:"'.$chart_name.'"});';
	}else if($chart_type == 'areachart'){
		$output .= $nl.'new google.visualization.AreaChart(document.getElementById("div_visualization")).draw(data, {is3D: true, min:0, title:"'.$chart_name.'"});';
	}else{ // columnchart
		$output .= $nl.'new google.visualization.ColumnChart(document.getElementById("div_visualization")).draw(data, {is3D: true, min:0, title:"'.$chart_name.'"});';
	}
