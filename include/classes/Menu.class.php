...
/**
 *	Class Menu (for Shopping Cart ONLY)
 *  -------------- 
...
class Menu {
	...
	/**
	 *	Draws all menus in dropdown box
	 *		@param $menu_id
	 *		@param $language_id
	 */
	...
	/**
	 *	Draw menus placement in dropdown box
	 *		@param $menu_placement
	 */
	...
	public static function DrawTopMenu()
	{
		...
		if(Application::Get('customer') == 'login' || Application::Get('customer') == 'my_account'){
			$customer_css = ' class="active"';
		}else if(Application::Get('page') == 'home'){
			$home_css = ' class="active"';
		}

		echo '<li><a'.$home_css.' href="index.php">'._HOME.'</a></li>';			

		if(Modules::IsModuleInstalled('customers')){			
			if(ModulesSettings::Get('customers', 'show_my_account_link') == 'yes'){
				echo '<li><a'.$customer_css.' href="index.php?customer=my_account">'._MY_ACCOUNT.'</a></li>';
			}
		}
	...
	public static function DrawFooterMenu()
	{
		...
		if(Modules::IsModuleInstalled('customers')){			
			if(ModulesSettings::Get('customers', 'show_my_account_link') == 'yes'){
				echo ' : <a href="index.php?customer=my_account">'._MY_ACCOUNT.'</a>';
			}
		}
		if(Modules::IsModuleInstalled('shopping_cart')){
			if(ModulesSettings::Get('shopping_cart', 'is_active') == 'yes'){
		...
		$menus = self::GetAllFooter('', Application::Get('lang'));
		if($menus[1] > 0) echo '<br />';
		for($menu_ind = 0; $menu_ind < $menus[1]; $menu_ind++){
			// Draw current menu link
			if($menu_ind > 0) echo ': ';
			echo prepare_link('pages', 'pid', $menus[0][$menu_ind]['id'], $menus[0][$menu_ind]['page_key'], $menus[0][$menu_ind]['menu_link'], '');
		}
	}
	...
	public static function DrawMenu($menu_position = 'left', $draw = true)
	{
		...
		if($menus_count > 0) $output .= '<div id="column-'.$menu_position.'-wrapper"'.(($menus_count > 0) ? ' class="column-wrapper-width"' : '').'>';

		$products_catalog_allowed = (ModulesSettings::Get('products_catalog', 'is_active') == 'yes') ? true : false;
		
		$shopping_cart_allowed = false;
		if(Modules::IsModuleInstalled('shopping_cart')){
			if(ModulesSettings::Get('shopping_cart', 'is_active') == 'yes'){
				if(ModulesSettings::Get('shopping_cart', 'payment_type_paypal') == 'yes' || ModulesSettings::Get('shopping_cart', 'payment_type_2co') == 'yes'){
					$shopping_cart_allowed = true;
				}
			}
		}
		
		if($products_catalog_allowed && ModulesSettings::Get('products_catalog', 'show_categories_side_block') == $menu_position.' side') { $output .= Categories::DrawSideBlock(false); $menus_count++; }
		if($products_catalog_allowed && ModulesSettings::Get('products_catalog', 'show_featured_block') == $menu_position.' side') { $output .= Products::DrawFeaturedBlock('side', false); $menus_count++; }		 
		if($products_catalog_allowed && ModulesSettings::Get('products_catalog', 'show_manufacturers_side_block') == $menu_position.' side') { $output .= Manufacturers::DrawSideBlock(false); $menus_count++; }
		
		// Display all menu titles (names) according to their order
		for($menu_ind = 0; $menu_ind < $menus[1]; $menu_ind++) {								
			// Start draw new menu
			$output .= draw_block_top($menus[0][$menu_ind]['menu_name'], '', 'maximized', false);			
		...
		if($menu_position == 'left'){
			...
			if($shopping_cart_allowed){
				...
				$output .= '<img src="images/ppc_icons/logo_paypal.gif" border="0" title="PayPal" alt="PayPal" />
		...
		if($products_catalog_allowed && ModulesSettings::Get('products_catalog', 'show_catalog_statistics') == $menu_position.' side') $output .= Products::DrawCatalogStatistics(false);				
	...
	/**
	 *	Draws menu accessible dropdown menu
	 *		@param $access_level
	 */
