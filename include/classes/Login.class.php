...
class Login {
	...
	private function DoLogin($user_name, $password, $remember_me = '', $do_redirect = true)
	{
		...
		if($account_information = $this->GetAccountInformation($user_name, $password)){
			...
			if($do_redirect){
				ShoppingCart::ReCalcualteDelivery($this->IsLoggedInAsCustomer(false), $this->GetLoggedID());
		...
		if($this->IsLoggedInAsAdmin()){
			...
			draw_block_top(_ACCOUNTS_MANAGEMENT, $menu_index++);
				...
				if(Modules::IsModuleInstalled('customers') && $this->IsLoggedInAs('owner','mainadmin')){
					...
					echo '<li>'.$this->PrepareLink('mod_customers_statistics', _STATISTICS).'</li>';
					echo '</ul>';
				}
				echo '</div>';
			draw_block_bottom();
			...
			if(Modules::IsModuleInstalled('products_catalog')){
				draw_block_top(_PRODUCTS_CATALOG, $menu_index++);
					echo '<div class="menu_category">';
					if($this->IsLoggedInAs('owner','mainadmin')){
						echo '<label>'._SETTINGS.'</label>';
						echo '<ul>';
						echo '<li>'.$this->PrepareLink('mod_catalog_currencies', _CURRENCIES).'</li>';
						echo '<li>'.$this->PrepareLink('mod_products_catalog_settings', _CATALOG_SETTINGS).'</li>';
						echo '</ul>';
					}
					echo '<label>'._PRODUCTS_MANAGEMENT.'</label>';
					echo '<ul>';
					echo '<li>'.$this->PrepareLink('mod_catalog_manufacturers', _MANUFACTURERS).'</li>';
					echo '<li>'.$this->PrepareLink('mod_catalog_categories', _CATEGORIES, '', '', array('mod_catalog_category_description')).'</li>';
					echo '<li>'.$this->PrepareLink('mod_catalog_products', _PRODUCTS, '', '', array('mod_catalog_product_description')).'</li>';
					echo '</ul>';
					echo '</div>';							
				draw_block_bottom();		
			}
			...
			if(Modules::IsModuleInstalled('shopping_cart')){				
				draw_block_top(_SHOPPING_CART, $menu_index++);
					echo '<ul>';
					echo '<li>'.$this->PrepareLink('mod_shop_delivery_settings', _DELIVERY_SETTINGS).'</li>';
					echo '<li>'.$this->PrepareLink('mod_shop_campaigns', _CAMPAIGNS).'</li>';
					echo '<li>'.$this->PrepareLink('mod_shop_orders', _ORDERS).'</li>';
					echo '<li>'.$this->PrepareLink('mod_shop_orders_statistics', _STATISTICS).'</li>';
					echo '</ul>';
				draw_block_bottom();				
			}
			...
			draw_block_top(_MY_ACCOUNT);
				...
				if(Modules::IsModuleInstalled('shopping_cart')){				
					if(ModulesSettings::Get('shopping_cart', 'is_active') == 'yes'){
						...
						echo '<li>'.$this->PrepareLink('my_products', _MY_PRODUCTS).'</li>';
					}
				}
				echo '</ul>';
				
				// logout
				echo '<form action="index.php" id="frmLogout" style="margin-top:5px;" method="post"><input type="hidden" name="submit_logout" value="logout" /></form>';
				echo '<a href="javascript:void(0);" onclick="javascript:appFormSubmit(\'frmLogout\')">'._BUTTON_LOGOUT.'</a>';
			draw_block_bottom();
			//draw_block_footer();
		}				
