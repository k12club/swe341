 ...
 *  1.8.3
 *      - added alignment for rtl languages
 *      - replaced input tag width:0px with hidden field
 *      - replaced @Languages::GetLanguageDirection($this->languageId); with Application::Get('lang_dir')
 *      - added if(Application::Get('lang_dir') == 'rtl') echo '&nbsp;'; to rtl languages
 *      - fixed syntax issue in debug for view mode
 *  1.8.2
 *      - added debug for view mode
 *      - fixed issue with using currencies that ends with a dot (.)
 *      - fixed bug with empty actions
 *      - added possibility to print
 *      - added '<th width="8%" align="center">'._ACTIONS.'</th>';
 *  1.8.1
 *      - added header_tooltip for details mode
 *      - removed doible drawing BeforeDetailsRecord()
 *      - added attribute "javascript_event" for filtering fields
 *      - changed text for image thumbnail tooltip
 *      - wrong drawing empty enum filtering field
 *  1.8.0
 *      - improved refresh operation
 *      - improved highlight() call
 *      - added $this->maxFilteringColumns = 4; 
 *      - changed mysql_error() with database_error();, mysql_insert_id() with database_insert_id()
 *      - mysql_real_escape_string() replaced with encode_text()
 *  1.7.9
 *      - added possibility to add watermark with: 'watermark'=>false, 'watermark_text'=>''
 *      - added alternative code (commented) to Mixing of GROUP columns SQL issue
 *      - small bug fixed in DrawEditModeButtons
 *      - improved pagination - for large number of records
 *      - added SetAlerts()
 *  1.7.8
...
class MicroGrid {
	...
	{        
			...
			if($search_status == 'active'){
				...
				foreach($this->arrFilteringFields as $key => $val){
					...
					if(!$custom_handler && self::GetParameter('filter_by_'.$val['table'].$val['field'], false) !== ''){
						...
						$this->WHERE_CLAUSE .= ' AND '.$field_name.' '.$sign.' \''.$sign_start.encode_text($key_value).$sign_end.'\' ';                        
					}
				}
			}			
		}		
		...
		if($this->isFilteringAllowed){
			...
			echo '<tr>'.$nl;
			echo '<td align="'.Application::Get('defined_left').'" width="65px"><b>'._FILTER_BY.'</b>: &nbsp;</td>'.$nl;
                ...
                foreach($this->arrFilteringFields as $key => $val){                            
                    ...
                    if($val['type'] == 'text'){
                        ...
                        if(is_array($val['source'])){
                            ...
                            echo $key.':&nbsp;<select class="mgrid_text" name="filter_by_'.$val['table'].$val['field'].'" '.(($javascript_event!='') ? ' '.$javascript_event : '').' style="width:'.$val['width'].'">';
		...
		if($arrRecords[1] > 0){            
			...
			if($action_details || $action_edit || $action_delete){
				echo '<th width="8%" align="center">'._ACTIONS.'</th>';
			}
			echo '</tr>';
			...
			if(preg_match('/separator/i', $key) && is_array($val)){
				...
				foreach($val as $v_key => $v_val){
					...
					if($v_key != 'separator_info' && $v_val['type'] != 'hidden'){
						...
						echo '<td width="25%" align="'.Application::Get('defined_left').'"><label for="'.$v_key.'">'.$v_val['title'].'</label>';
				...
				if($val['type'] != 'hidden'){
					...
					echo '<td width="25%" align="'.Application::Get('defined_left').'"><label for="'.$key.'">'.$val['title'].'</label>';
		...
		if($mode == 'view'){            
			...
			switch($field_type){
				case 'link':
					...
					$output = '<a href="'.$href_str.'"'.$target_str.' title="'.strip_tags($title).'">'.$field_value.'</a>'.$post_html;
					break;
	...
	*/	
	...
	private function DrawEditModeButtons($buttons = array('reset'=>false, 'cancel'=>true))
	{
		...
		echo '<tr>
				<td colspan="2" align="'.Application::Get('defined_left').'">
					...
					'.((isset($buttons['reset']) && $buttons['reset']) ? '<input class="mgrid_button" type="reset" name="btnReset" value="'._BUTTON_RESET.'" />&nbsp;' : '').'
					'.((isset($buttons['cancel']) && $buttons['cancel']) ? '<input class="mgrid_button" type="button" name="btnCancel" value="'._BUTTON_CANCEL.'" onclick="javascript:__mgDoPostBack(\''.$this->tableName.'\',\'view\');" />' : '').'
				</td>
			  <tr>'.$nl;		
	}
    ...
    public function AddTranslateToModes($translations = array(), $params = array())
	{
		foreach($translations as $key => $val){			
			$this->arrAddModeFields['separator_'.$key]['separator_info'] = array('legend'=>'<img src="images/flags/'.$val['icon_image'].'" alt="flag" />&nbsp;&nbsp;'.$val['lang_name']);
			...
			$this->arrEditModeFields['separator_'.$key]['separator_info'] = array('legend'=>'<img src="images/flags/'.$val['icon_image'].'" alt="flag" />&nbsp;&nbsp;'.$val['lang_name']);
			foreach($params as $p_key => $p_val){
			...
			$this->arrDetailsModeFields['separator_'.$key]['separator_info'] = array('legend'=>'<img src="images/flags/'.$val['icon_image'].'" alt="flag" />&nbsp;&nbsp;'. $val['lang_name']);
			foreach($params as $p_key => $p_val){
