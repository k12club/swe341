...
class PDF extends FPDF
{
    ...
    function FancyTable($header,$data)
    {
        ...
        foreach($data as $row)
        {
            ...
            if(isset($row[1])) $this->Cell($w[1],6,$row[1],'LR',0,'L',$fill);
            if(isset($row[2])) $this->Cell($w[2],6,number_format($row[2]),'LR',0,'R',$fill);
            if(isset($row[3])) $this->Cell($w[3],6,number_format($row[3]),'LR',0,'R',$fill);
            $this->Ln();
            $fill=!$fill;
        }
        $this->Cell(array_sum($w),0,'','T');
