...
$title_bar = '<table width="99%" align="center" cellspacing="0" cellpadding="0">
	<tr>
		<td align="'.Application::Get('defined_left').'" style="font:bold 15px arial, sans-serif;">'._SEARCH_RESULT_FOR.': '.$keyword.'</td>
		<td align="'.Application::Get('defined_right').'">
			'._LOOK_IN.':
			<select name="search_in" onchange="javascript:document.getElementById(\'search_in\').value=this.value;appQuickSearch();">
				<option value="products" '.((Application::Get('search_in') == 'products') ? 'selected="selected"' : '').'>'._IN_PRODUCTS.'</option>
				<option value="pages" '.((Application::Get('search_in') == 'pages') ? 'selected="selected"' : '').'>'._IN_PAGES.'</option>
